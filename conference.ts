 ...
 * cover the configured-conference object including aliases, PINs, host
 * and guest views, themes, crypto mode, and live captions. Gateway and
 * call-routing rule helpers manage alias matching for inbound and
 * outbound legs. Active-conference commands (lock, mute, transfer,
 * disconnect, role change, layout transform, mute-all-guests) write
 * directly to the platform's command endpoint, so they take effect on
 * in-progress calls. History helpers pull conference-level and
 * participant-level CDRs with per-stream media stats. Alias and
 * automatic-participant CRUD covers the secondary objects most often
 * touched alongside VMR provisioning — RTMP streaming targets,
 * always-on endpoints, scheduled and recurring conferences.
 */
export const model = {
