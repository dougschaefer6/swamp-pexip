...
/**
 * `@dougschaefer/pexip-exchange` model — Microsoft Exchange integration
 * surface for Pexip Infinity on the v39 management API. Connector CRUD
 * (listConnectors, getConnector, createConnector, updateConnector,
 * deleteConnector) manages the EWS or Graph connection Infinity uses
 * to read scheduled meetings from Exchange resource mailboxes for
 * One-Touch Join. listScheduledAliases returns the calendar-derived
 * dial strings Infinity has materialized as conference aliases.
 * Scaling-policy methods (list/create/delete) tie connector capacity
 * and refresh cadence to platform load. inventory rolls the Exchange
 * integration posture together for audit. Mutations affect live
 * One-Touch Join behavior across all rooms that depend on the
 * connector — schedule cutovers carefully.
 */
export const model = {
