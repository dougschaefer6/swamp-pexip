...
/**
 * `@dougschaefer/pexip-platform` model — platform-wide operations on
 * Pexip Infinity over the v39 management API. getConfig and
 * updateConfig drive the global system-configuration object (default
 * theme, support contacts, time zone, retention policy). listNodes
 * and getNodeStatus enumerate management, conferencing, and proxying
 * nodes with health and version. setMaintenanceMode drains a node
 * for upgrade. DNS-server and NTP-server CRUD configure platform
 * resolution and time sync. Location CRUD (listLocations,
 * createLocation) defines the geographic groupings that drive media
 * routing and cascade selection. License, alarm, and backup
 * surfaces are also wired here (getLicenseStatus, listAlarms,
 * createBackup, restoreBackup, listBackups). upgrade triggers the
 * platform upgrade workflow; takeSnapshot captures support
 * diagnostics. dialParticipant and disconnectConference are
 * platform-scope wrappers over the command API.
 * startCloudNode brings a cloud-burst conferencing node online.
 * Mutations affect production media services across the entire
 * cluster — pair with change windows.
 */
export const model = {
